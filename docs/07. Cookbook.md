...
This code protects your service so that only users that have the `admin` role can delete a post. However, this allow
...
can either be callable (closure...) or any class that implements `ZfcRbac\Assertion\AssertionInterface`.
...
Each assertion gives you access to the authorization service. Here is an example:
...
public function deletePost($id)
{
    ...
    $assertion = function(AuthorizationService $authorizationService) {
        return $authorizationService->getIdentity() === $post->getAuthor();
    };
