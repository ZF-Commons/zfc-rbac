...
class User implements IdentityInterface
{
    /**
     * @var Collection
     * @ORM\ManyToMany(targetEntity="HierarchicalRole", fetch="LAZY")
     */
    private $roles;
    ...
    public function setRoles(Collection $roles = null)
    {
        $this->roles = $roles;
...
And the last step is to create Permission entity class which is a very simple entity class, you don't have to doing specific things !

You can find all entity example in this folder : [Example](/data)

You need one more configuration step. Indeed, how the RoleProvider can retrieve your role and permissions ? For this you need to configure `ZfcRbac\Role\ObjectRepositoryRoleProvider` in your `zfc_rbac.global.php` file :
```php
        /**
...
Using DoctrineORM with ZfcRbac is very simple. You need to take aware from performances where there is a lot of permissions for roles.
