...
class MustBeAuthorAssertion implements ContextAwareAssertionInterface
{
    /**
    ...
    {
        return $authorization->getIdentity() === $context->getContext()->getAuthor();
    }
}
```
...
class MustBeAuthorAssertion implements ContextAwareAssertionInterface
{
    ...
    public function assert(AuthorizationServiceInterface $authorization, AuthorizationContextInterface $context)
    {
        if ($authorization->getIdentity() === $context->getContext()->getAuthor()) {
            return true;
        }
...
Using the `AuthorizationContextInterface` you can normalize this step. Let's take the `deletePost` example.
In case you would want to implement a `deleteComment` permission as well,
you would have to write two assertions implementing the same logic.
If you normalize the naming, for example `deleteComment`, `deletePost`, `deleteCommentByEveryone`,
`deletePostByEveryone`, you will only need to implement one simple assertion:
...
class MustBeAuthorAssertion implements ContextAwareAssertionInterface
{
    ...
    public function assert(AuthorizationServiceInterface $authorization, AuthorizationContextInterface $context)
    {
        ...
        $permission = $context->getPermission() . 'ByOthers' ;
...
Calling `$authorizationService->isGranted('deleteComment`, $comment)` will check for the permission
`deleteCommentByOthers` if the user is not the author.
